component accessors="true" {
	 ...
	 */
	public AbsRestApi function init(required string baseUrl, string apiKey = "", string authToken = "", struct defaultHeaders = {}) {
		setBaseUrl(baseUrl);
		setApiKey(apiKey);
	...
	public struct function call(
		...
		if (len(getAuthToken())) {
			if (find("?", url)) {
				url &= "&token=" & urlEncodedFormat(getAuthToken());
			} else {
				url &= "?token=" & urlEncodedFormat(getAuthToken());
			}
		}
		...
		// Log richiesta
		writeLog(type="information", text="API Request: #method# #url# | Headers: #serializeJson(requestHeaders)# | Body: #left(body, 500)#");

		// Fai la chiamata
		cfhttp(url=url, method=method, timeout=getTimeout()) {
		...
		// Log risposta
		writeLog(type="information", text="API Response: #response.statusCode# | Headers: #serializeJson(response.headers)# | Body: #left(response.body, 500)#");
	 ...
	 * Helper per GET
	 */
	public any function get(required string endpoint, struct params = {}, struct headers = {}) {
	 ...
	 * Helper per POST
	 */
	public any function post(required string endpoint, any data = "", struct headers = {}) {
	 ...
	 * Helper per PUT
	 */
	public any function put(required string endpoint, any data = "", struct headers = {}) {
	 ...
	 * Helper per PATCH
	 */
	public any function patch(required string endpoint, any data = "", struct headers = {}) {
