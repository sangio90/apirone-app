...
CREATE TABLE public.export_code_raw_values (
  ...
  CONSTRAINT export_codes_raw_value_id_fk FOREIGN KEY (raw_value_id)
    ...
    ON UPDATE CASCADE
    NOT DEFERRABLE,
  CONSTRAINT export_codes_attribute_id_fk FOREIGN KEY (attribute_id)
    REFERENCES public.attributes (attribute_id)
    ON DELETE NO ACTION
    ON UPDATE CASCADE
    NOT DEFERRABLE,
  CONSTRAINT export_codes_export_code_raw_values_id_fk FOREIGN KEY (export_code_id)
    REFERENCES public.export_codes (export_code_id)
    ON DELETE CASCADE
    ON UPDATE CASCADE
    NOT DEFERRABLE
)
