CREATE TABLE public.catalog_sets (
    catalog_set_id UUID STORAGE PLAIN DEFAULT uuid_generate_v4 () NOT NULL,
    catalog_set VARCHAR(125) STORAGE PLAIN,
    ...
    CONSTRAINT catalog_sets_pkey PRIMARY KEY (catalog_set_id),
    CONSTRAINT catalog_sets_line_id_fk FOREIGN KEY (line_id) REFERENCES public.lines (line_id) ON DELETE NO ACTION ON UPDATE CASCADE NOT DEFERRABLE,
    CONSTRAINT catalog_sets_model_id_fk FOREIGN KEY (model_id) REFERENCES public.models (model_id) ON DELETE NO ACTION ON UPDATE CASCADE NOT DEFERRABLE,
    CONSTRAINT catalog_sets_product_category_id_fk FOREIGN KEY (product_category_id) REFERENCES public.product_categories (product_category_id) ON DELETE NO ACTION ON UPDATE CASCADE NOT DEFERRABLE
);

CREATE UNIQUE INDEX catalog_sets_idx ON public.catalog_sets USING btree (line_id, model_id, product_category_id);

ALTER TABLE public.catalog_sets OWNER TO apiruser;

ALTER TABLE public.catalog_sets
ALTER COLUMN catalog_set
SET
    STORAGE PLAIN;

CREATE TABLE public.signage_configs (
    signage_config_id SERIAL,
    catalog_set_id UUID STORAGE PLAIN,
    ...
    CONSTRAINT signage_configs_catalog_set_id_fk FOREIGN KEY (catalog_set_id) REFERENCES public.catalog_sets (catalog_set_id) ON DELETE NO ACTION ON UPDATE CASCADE NOT DEFERRABLE
);
